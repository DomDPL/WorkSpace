...
namespace DPLRef.eCommerce.Tests.ManagerTests
{
    ...
    public class CatalogManagerTests : ManagerTestBase
    {
        ...
        private MockCatalogAccessor MockCatalogAccessor { get; set; }

        public WebStore.IWebStoreCatalogManager CreateWebStoreCatalogManager()
        {
            var factory = new ManagerFactory(mockData.Context);
            var manager = factory
                .CreateManager<WebStore.IWebStoreCatalogManager>(
                    null, SetupMockAccessorFactory(), SetupMockUtilityFactory());
            return manager;
        }

        public Admin.IAdminCatalogManager CreateAdminCatalogManager()
        {
            var factory = new ManagerFactory(mockData.Context);
            var manager = factory
                .CreateManager<Admin.IAdminCatalogManager>(
                    null, SetupMockAccessorFactory(), SetupMockUtilityFactory());
            return manager;
        }
        ...
        public UtilityFactory SetupMockUtilityFactory()
        {
            ...
            var mockSecurity = new MockSecurityUtility(mockData);
            utilFactory.AddOverride<ISecurityUtility>(mockSecurity);
            return utilFactory;
        }

        private static Admin.WebStoreCatalog CreateWebStoreCatalog(int catalogId = 0) => new()
        {
            Id = catalogId,
        ...
        public void CatalogManager_AdminSaveProduct_Fail()
        {
            ...
            var response = mgr.SaveProduct(1, new Admin.Product
            {
                Detail = "UNIT TEST PRODUCT",
                IsAvailable = true,
                Price = 10.0m,
