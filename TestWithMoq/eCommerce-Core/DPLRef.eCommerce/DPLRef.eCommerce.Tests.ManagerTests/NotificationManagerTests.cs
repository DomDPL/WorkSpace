...
namespace DPLRef.eCommerce.Tests.ManagerTests
{
    ...
    public class NotificationManagerTests : ManagerTestBase
    {
        ...
        private INotificationManager GetNotificationManager()
        {
            mockData.Orders.Add(
                new Order
                {
                    ...
                    OrderLines = System.Array.Empty<OrderLine>()
                });
        ...
        public void NotificationManager_SendNewOrderNotices()
        {
            ...
            Assert.AreEqual($"New Order Notice email sent to {_testOrderEmailAddress} for order {_testOrderId}",
                response.Message);
        }
        ...
        public void NotificationManager_SendShippingNotices()
        {
            ...
            Assert.IsTrue(response.Success);
            Assert.AreEqual($"Shipping Notice email sent to {_testOrderEmailAddress} for order {_testOrderId}",
                response.Message);
        }
