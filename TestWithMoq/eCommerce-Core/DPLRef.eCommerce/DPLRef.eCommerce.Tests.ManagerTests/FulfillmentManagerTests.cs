...
namespace DPLRef.eCommerce.Tests.ManagerTests
{
    ...
    public class FulfillmentManagerTests : ManagerTestBase
    {
        ...
        private static readonly Address _validAddress = new()
        {
            Addr1 = "Address 1",
            City = "City",
            First = "First",
        ...
        private readonly Order _orderToFulfill = new()
        {
            Id = _goodOrderId,
            TaxAmount = 0.11m,
            SubTotal = 1.50m,
        ...
        private IAdminFulfillmentManager GetFulfillmentManager(string authToken)
        {
            ...
            var mgrFactory = new ManagerFactory(mockData.Context);
            return mgrFactory.CreateManager<IAdminFulfillmentManager>(null, SetupMockAccessorFactory(),
                SetupMockUtilityFactory());
        }
        ...
        private AccessorFactory SetupMockAccessorFactory()
        {
            ...
            var mockShippingAccessor = new MockShippingAccessor(mockData);

            var accFactory = new AccessorFactory(mockData.Context, new UtilityFactory(mockData.Context));
        ...
        private UtilityFactory SetupMockUtilityFactory()
        {
            ...
            var utilFactory = new UtilityFactory(mockData.Context);
        ...
        public void FulfillmentManager_FulfillOrder()
        {
            ...
            mockData.ShippingResult = new ShippingResult
            {
                Success = true,
            ...
            Assert.IsTrue(mockData.OrderCaptureAttempted);
            Assert.IsTrue(mockData.OrderCapturedStatus);
            Assert.IsTrue(mockData.OrderShippingRequested);
            Assert.IsTrue(mockData.OrderFulfilled);
            // verify the final status of the order
            var order = mockData.Orders.First(o => o.Id == _goodOrderId);
        ...
        public void FulfillmentManager_FulfillOrderNoAuth()
        {
            ...
            Assert.IsFalse(mockData.OrderCaptureAttempted);
            Assert.IsFalse(mockData.OrderCapturedStatus);
            Assert.IsFalse(mockData.OrderShippingRequested);
            Assert.IsFalse(mockData.OrderFulfilled);
            //make sure send event called
