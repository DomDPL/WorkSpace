...
namespace DPLRef.eCommerce.Managers.Notification
{
    internal class NotificationManager : ManagerBase, INotificationManager
    {
        ...
        public NotificationResponse SendNewOrderNotices(int orderId) => SendOrderEventEmail(orderId, "New Order Notice");

        public NotificationResponse SendOrderFulfillmentNotices(int orderId) => SendOrderEventEmail(orderId, "Shipping Notice");
        ...
        private NotificationResponse SendOrderEventEmail(int orderId, string eventMessage)
        {
            try
            {
                ...
                AccessorFactory.CreateAccessor<IEmailAccessor>()
                    .SendEmailNotification(order.BillingAddress.EmailAddress, $"{eventMessage}: {seller.Name}",
                        messageBody);
