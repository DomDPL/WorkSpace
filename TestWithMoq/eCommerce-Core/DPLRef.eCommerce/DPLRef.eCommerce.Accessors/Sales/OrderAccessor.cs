...
{
    internal class OrderAccessor : AccessorBase, IOrderAccessor
    {
            ...
            using (var db = eCommerceDbContext.Create())
            {
                ...
                if (isNewOrder)
                {
                    foreach (var line in order.OrderLines)
                    {
                        ...
                        _ = db.OrderLines.Add(orderLineModel);
        ...
        public Order FindOrder(int id)
        {
            ...
            if (id > 0)
            {
                ...
                if (model != null)
                {
                    ...
                    var orderLines = new List<DataTransferObjects.OrderLine>();

                    foreach (var cim in orderItemModels)
                    {
                        var orderLine = DTOMapper.Map<DataTransferObjects.OrderLine>(cim);
        ...
        public SellerSalesTotal SalesTotal()
        {
            ...
            using (var db = eCommerceDbContext.Create())
            {
                result = (from o in db.Orders
                    ...
                    group o by new {o.SellerId, s.Name}
                    into g
        ...
        public Order[] UnfulfilledOrders()
        {
            ...
            using (var db = eCommerceDbContext.Create())
            {
                var list = from o in db.Orders
                    where o.SellerId == Context.SellerId && o.Status == OrderStatuses.Authorized
                    select o.Id;
