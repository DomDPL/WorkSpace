...
{
    internal class ValidationEngine : EngineBase, IOrderValidationEngine
    {
        #region IOrderValidation

        public ValidationResponse ValidateOrder(Order order)
        {
            var response = new ValidationResponse {Success = false};
            ...
            {
                var lineTotal = orderLine.UnitPrice * orderLine.Quantity;
